...
$sql1="SELECT * FROM level5 WHERE tek_emailid='$email'";
...
echo $flag."  ".$patients_saved." ".$time."  ".$health;
  ...
  if($flag==1)
 {
	 ...
	 $sql="select tek_patients_saved,tek_time  from level5 where tek_emailid ='$email'";
	 ...
	 if($patients_saved==$rows[0])
	 {
		if($time>$rows[1])
        {
			$sql2="UPDATE level5 SET tek_score='$score',tek_health='$health' ,tek_time='$time', tek_patients_saved='$patients_saved'
	       WHERE tek_emailid ='$email'";
           $result2=mysql_query($sql2) or die(mysql_error());
           if(mysql_affected_rows()==1)
           echo "level 5 same score with better time updated"."<br>";
           else
           echo "level 5 same score but less time : not updated"."<br>";
...
{
$sql2 = "INSERT INTO level5 (tek_emailid,tek_patients_saved,tek_time,tek_health,tek_score) VALUES ('$email','$patients_saved','$time','$health','$score')";
$result2=mysql_query($sql2)or die(mysql_error());
if(mysql_affected_rows()==1)
